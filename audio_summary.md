...
## 移动端音频可能存在的坑及解决方案
  ...
  * 属性相关
    ...
    * muted 
      ...
      注: 声音加载完成后设置才有效, 正在播放的音频设置不会立即生效, 暂停后再次播放生效 (静音)
    ...
    * preload [ none | metadata | auto ]
      ...
      移动端除非由用户接触事件, 比如 onclick, ontouchstart 触发事件, 否则不会加载音频流.

      解决方案: 

      支持 Web Audio API 可以采用同 autoplay 同样的方法, 不支持的目前还没发现好的解决方案. [探索下AJAX]
  ...
  * 方法相关
    ...
    * play()
      ...
      安卓 Chrome35     需要在交互事件函数中调用 (如 click, touchstart), 之后的调用后无调用限制
...
## 总结
  ...
  加载和并行播放是两个问题稍微多一些的地方, 针对自动加载后播放及其他一些小的兼容性问题推荐使用 howler.js 音频库, 
